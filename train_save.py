...
#     acc_steps        = max(1, int(getattr(config.TRAIN, "ACC_STEPS", 1)))
#     clip_max_norm    = float(getattr(config.TRAIN, "CLIP_GRAD_NORM", 5.0))
#     use_amp          = bool(getattr(config.TRAIN, "AMP", False))

#     # 两个优化器 → 两个 scaler
#     if use_amp:
#         if not hasattr(train_cal, "_scaler_main"):
#             train_cal._scaler_main = GradScaler(init_scale=2**12, growth_factor=2.0,
#                                                 backoff_factor=0.5, growth_interval=200)
#         if not hasattr(train_cal, "_scaler_cc"):
#             train_cal._scaler_cc = GradScaler(init_scale=2**12, growth_factor=2.0,
#                                               backoff_factor=0.5, growth_interval=200)
#         scaler_main = train_cal._scaler_main
#         scaler_cc   = train_cal._scaler_cc

#     logger.info(f"[ERASE] keep_ratio_range={erase_keep_range}, dilate_px={erase_dilate_px}, erase_prob={erase_prob}")
#     logger.info(f"[LOSS WEIGHTS] pair={config.LOSS.PAIR_LOSS_WEIGHT}, adv={-1 if epoch < config.TRAIN.START_EPOCH_ADV else adv_weight}, att={att_weight}")

#     end = time.time()
#     optimizer.zero_grad(set_to_none=True)
#     optimizer_cc.zero_grad(set_to_none=True)
...
#         # ===== 衣服分类支路：前向可在 autocast，loss 在 FP32 计算 =====
#         if use_amp:
#             with autocast():
#                 pred_clothes_raw = clothes_classifier(features_raw.detach())
#                 new_pred_clothes = clothes_classifier(features_raw)
#             # clothes_loss / adv_loss 在 FP32 中算，稳定
#             with autocast(enabled=False):
#                 clothes_loss = criterion_clothes(pred_clothes_raw.float(), clothes_ids)
#                 if epoch >= config.TRAIN.START_EPOCH_ADV:
#                     adv_loss = criterion_adv(new_pred_clothes.float(), clothes_ids, pos_mask)
#                 else:
...
#             if use_amp:
#                 with autocast(enabled=False):
#                     att_loss = build_attention_loss(features_raw.float(), M_face.float(), M_limbs.float(), M_hair.float())
#             else:
...
#         # ===== 总损失（主优化器）=====
#         if epoch >= config.TRAIN.START_EPOCH_ADV:
#             loss_main = cla_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss + adv_weight * adv_loss + att_weight * att_loss
#         else:
#             loss_main = cla_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss + att_weight * att_loss

#         loss_main_scaled = loss_main / acc_steps
#         if use_amp:
#             scaler_main.scale(loss_main_scaled).backward()
#         else:
#             loss_main_scaled.backward()

#         # ===== 衣服分类器优化器（第二个优化器）=====
#         if epoch >= config.TRAIN.START_EPOCH_CC:
#             clothes_loss_scaled = clothes_loss / acc_steps
#             if use_amp:
#                 scaler_cc.scale(clothes_loss_scaled).backward()
#             else:
#                 clothes_loss_scaled.backward()

#         # ===== 累计 step / 梯度裁剪 / step & update =====
#         do_step  = ((batch_idx + 1) % acc_steps == 0)
#         last_iter = (batch_idx + 1 == len(trainloader))

#         if do_step or last_iter:
#             if use_amp:
#                 # 主优化器
#                 scaler_main.unscale_(optimizer)
#                 torch.nn.utils.clip_grad_norm_(list(model.parameters()) + list(classifier.parameters()), max_norm=clip_max_norm)
#                 scaler_main.step(optimizer)
#                 scaler_main.update()
#                 optimizer.zero_grad(set_to_none=True)
#                 # 衣服分类器
#                 if epoch >= config.TRAIN.START_EPOCH_CC:
#                     scaler_cc.unscale_(optimizer_cc)
#                     torch.nn.utils.clip_grad_norm_(clothes_classifier.parameters(), max_norm=clip_max_norm)
#                     scaler_cc.step(optimizer_cc)
#                     scaler_cc.update()
#                     optimizer_cc.zero_grad(set_to_none=True)
#                 else:
#                     optimizer_cc.zero_grad(set_to_none=True)
...
#     adv_weight       = getattr(config.LOSS, "ADV_WEIGHT", 1.0)
#     att_weight       = getattr(config.LOSS, "ATT_WEIGHT", 1.0)
#     acc_steps        = max(1, int(getattr(config.TRAIN, "ACC_STEPS", 2)))
#     clip_max_norm    = float(getattr(config.TRAIN, "CLIP_GRAD_NORM", 5.0))
#     use_amp          = bool(getattr(config.TRAIN, "AMP", False))

#     if use_amp:
#         if not hasattr(train_cal_with_memory, "_scaler"):
#             train_cal_with_memory._scaler = GradScaler(init_scale=2**12, growth_factor=2.0,
#                                                        backoff_factor=0.5, growth_interval=200)
#         scaler = train_cal_with_memory._scaler
...
#         # 对抗/衣物记忆的 adv 损失：在 FP32 里算稳妥
#         if epoch >= config.TRAIN.START_EPOCH_ADV:
#             if use_amp:
#                 with autocast(enabled=False):
#                     adv_loss = criterion_adv(features_raw.float(), clothes_ids, pos_mask)
#             else:
...
#         # ===== 总损失 / 反传 =====
#         loss = cla_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss + adv_weight * adv_loss + att_weight * att_loss
...
def train_cal(config, epoch, model, classifier, clothes_classifier,
    ...
    erase_prob       = getattr(config.LOSS, "ERASE_PROB", 0.9)
    erase_keep_range = tuple(getattr(config.LOSS, "ERASE_KEEP_RANGE", (0.05, 0.15)))
    erase_dilate_px  = int(getattr(config.LOSS, "ERASE_DILATE_PX", 1))
    adv_weight       = getattr(config.LOSS, "ADV_WEIGHT", 0.5)
    att_weight       = getattr(config.LOSS, "ATT_WEIGHT", 1.0)
    acc_steps        = max(1, int(getattr(config.TRAIN, "ACC_STEPS", 2)))
    ...
    for batch_idx, batch in enumerate(trainloader):
        ...
        loss_main_scaled = loss_main / acc_steps
        if config.TRAIN.AMP:
            with amp.scale_loss(loss_main_scaled, optimizer) as scaled_loss:
        ...
        if epoch >= config.TRAIN.START_EPOCH_CC:
            clothes_loss_scaled = (clothes_loss / acc_steps)
            if config.TRAIN.AMP:
                with amp.scale_loss(clothes_loss_scaled, optimizer_cc) as scaled_loss:
                    scaled_loss.backward()
            else:
                clothes_loss_scaled.backward()
...
def train_cal_with_memory(config, epoch, model, classifier,
    ...
    for batch_idx, batch in enumerate(trainloader):
        ...
        loss_scaled = loss / acc_steps
        if config.TRAIN.AMP:
            with amp.scale_loss(loss_scaled, optimizer) as scaled_loss:
                scaled_loss.backward()
        else:
            loss_scaled.backward()
