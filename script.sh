...
# For LTCC dataset
python -m torch.distributed.launch --nproc_per_node=2 --master_port 12345 main.py --dataset ltcc --cfg configs/res50_cels_cal.yaml --gpu 1,2 #
# python -m torch.distributed.launch --nproc_per_node=2 --master_port 12345 main.py --dataset ltcc --cfg configs/res50_cels_cal.yaml --gpu 0,1  LOSS.ERASE_KEEP_RANGE [0.05,0.15] LOSS.ADV_WEIGHT 1.0 LOSS.ATT_WEIGHT 2.0
# For PRCC dataset
# python -m torch.distributed.launch --nproc_per_node=2 --master_port 12345 main.py --dataset prcc --cfg configs/res50_cels_cal.yaml --gpu 1,2 #

# For VC-Clothes dataset. You should change the root path of '--resume' to your output path.
# python -m torch.distributed.launch --nproc_per_node=2 --master_port 12345 main.py --dataset vcclothes --cfg configs/res50_cels_cal.yaml --gpu 1,2 #
...
# python -m torch.distributed.launch --nproc_per_node=2 --master_port 12345 main.py --dataset last --cfg configs/res50_cels_cal_tri_16x4.yaml --gpu 0,1 #
# # For CCVID dataset
