...
def build_dataset(config):
    if config.DATA.DATASET not in __factory.keys():
        raise KeyError("Invalid dataset, got '{}', but expected to be one of {}".format(
            config.DATA.DATASET, __factory.keys()))
...
def build_img_transforms(config):
    """
    关掉随机几何增强：只做确定性 Resize + ToTensor + Normalize
    RandomErasing 用配置 config.AUG.RE_PROB 控制；<=0 时禁用
    """
    # 基础确定性变换
    base_train = [
        T.Resize((config.DATA.HEIGHT, config.DATA.WIDTH)),
    ...
    # 根据配置启用/禁用 RandomErasing（不属于几何增强，可作为像素正则）
    re_prob = getattr(config.AUG, 'RE_PROB', 0.0)
    if re_prob and re_prob > 0:
        base_train.append(T.RandomErasing(probability=re_prob))

    transform_train = T.Compose(base_train)
    ...
    print(">>> TEST  TRANSFORM:", transform_test)

    return transform_train, transform_test


def build_vid_transforms(config):
    # 这里是视频数据集的空间/时间增强；你当前用 prcc（图像），这部分无需改动
    spatial_transform_train = ST.Compose([
        ST.Scale((config.DATA.HEIGHT, config.DATA.WIDTH), interpolation=3),
        ST.RandomHorizontalFlip(),
        ST.ToTensor(),
...
def build_dataloader(config):
    ...
    # image dataset
        ...
        trainloader = DataLoaderX(
            dataset=ImageDataset(dataset.train, transform=transform_train),
            sampler=train_sampler,
            batch_size=config.DATA.TRAIN_BATCH, num_workers=config.DATA.NUM_WORKERS,
            pin_memory=True, drop_last=True)
